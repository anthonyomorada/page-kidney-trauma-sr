...
write.csv(table1, "data/table1.csv", row.names = FALSE)
```

```{r flextable}
table_for_pub <- table1 %>%
  mutate(Year = as.numeric(gsub("[^0-9]", "", as.character(Year)))) %>%
  arrange(Year) %>%
  select(
    Reference,
    Year,
...
ft <- flextable(table_for_pub) %>%
  set_header_labels(
    ...
    `Size (cm)`        = "Size",
    `Page Kidney Type` = "PK Type",
    Laterality         = "Laterality",
    Treatment          = "Treatment",
    `Follow-up Duration` = "F/U Time"
  ) %>%
...
# Export to Word
read_docx() %>%
  body_add_par("Table X. Summary of Included Cases", style = "heading 1") %>%
  body_add_flextable(ft) %>%
  print(target = "page_kidney_table.docx")
```
